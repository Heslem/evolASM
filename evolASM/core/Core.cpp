...
const sf::Vector2f Core::getMousePosition() const
{
    return _window->mapPixelToCoords(sf::Mouse::getPosition(*_window), _camera->getView());
}

const sf::Vector2u& Core::getWindowSize() const
{
...
{
#ifdef _DEBUG
    sf::String title("evolASM! [DEBUG]");
#else
    sf::String title("evolASM!");
#endif

    Logger::info("Initialize renderer.");
    _window = new sf::RenderWindow(sf::VideoMode(1200, 720), title);
    ...
    io.Fonts->AddFontFromFileTTF("content/fonts/font.ttf", 10, NULL, io.Fonts->GetGlyphRangesCyrillic());
...
void Core::loop()
{
    ...
    Logger::info("Free renderer resources.");
    delete _window;
    delete _camera;
}
